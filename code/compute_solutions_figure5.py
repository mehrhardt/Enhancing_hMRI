# create results as shown in figure 5 of
# M. J. Ehrhardt, F. A. Gallagher, M. A. McLean, and C.-B. Schoenlieb, 
# Enhancing the Spatial Resolution of Hyperpolarized Carbon-13 MRI of Human 
# Brain Metabolism using Structure Guidance, MRM, 2021.

# load modules
import numpy as np
import os
import misc

# select data sets
names = ['3DInSilico_Pyruvate_40']

# select parameters
alphas_2ddtv = [1e-2] # regularization parameter
alphas_3ddtv = [1e-2] # regularization parameter
alphas_dwork2021 = [1e-4, 1e-2] # regularization parameter
gammas = [0.9995] # parameter in dtv
etas = [1e-2] # parameter in dtv
scalings = [100] # algorithm parameter
niter = 1000 # algorithm parameter

# process all data with "2d guide" for all parameters using dtv and Dwork2021
for name in names:
    name2d = '2' + name[1:]
    print('=== Dataset {}'.format(name2d))
    
    # set and create out folder
    folder_out = '../pics/figure5/{}'.format(name2d)    
    if not os.path.exists(folder_out):
        os.makedirs(folder_out)

    # load data
    dic = np.load('../processed_data/{}.npz'.format(name), allow_pickle=True)
    data = dic['data']
    guide = dic['guide']
    molecule = dic['molecule']
    seg = dic['seg']
    gtruth = dic['gt']
    
    c = guide.shape[2] // 2
    guide = guide[:,:,c]
    seg = seg[:,:,c]
    
    if gtruth is not None:
        gtruth = gtruth[:,:,c]
    
    # run recon for all parameters
    for gamma in gammas:
        for alpha in alphas_2ddtv: 
            for eta in etas:
                for scaling in scalings:
                    misc.recon_dtv(guide, data, seg, folder_out, alpha, eta, 
                                   gamma, niter, molecule, scaling=scaling,
                                   gtruth=gtruth)        

    # process all data using the method of Dwork2021
    for alpha in alphas_dwork2021: 
        for scaling in scalings:
            misc.recon_Dwork2021(guide, data, seg, folder_out, alpha, niter, 
                                 molecule, scaling=scaling, gtruth=gtruth)   
            
# process all data with "3d guide"
for name in names:
    print('=== Dataset {}'.format(name))
    
    # set and create out folder
    folder_out = '../pics/figure5/{}'.format(name)    
    if not os.path.exists(folder_out):
        os.makedirs(folder_out)

    # load data
    dic = np.load('../processed_data/{}.npz'.format(name), allow_pickle=True)
    data = dic['data']
    guide = dic['guide']
    molecule = dic['molecule']
    seg = dic['seg']
    gtruth = dic['gt']
    
    # run dtv recon with "3d guide" for all parameters
    for gamma in gammas:
        for alpha in alphas_3ddtv: 
            for eta in etas:
                for scaling in scalings:
                    misc.recon_dtv(guide, data, seg, folder_out, alpha, eta, 
                                   gamma, niter, molecule, scaling=scaling,
                                   gtruth=gtruth)
